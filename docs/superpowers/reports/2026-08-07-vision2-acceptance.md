...
1. **Качество lite-семантики**: SmolVLM2-500M даёт повторяющиеся caption'ы и галлюцинации
   в транскрипции. Рычаг — quality-пак `moondream/moondream2-gguf` (F16, ~3.7 ГБ диск,
   ~4 ГБ RAM вместо спековских ~2.6 ГБ — публичных Q4-квантов moondream2 не существует).
   Скачивание: `scripts/download-vision-models.py --pack quality`, включение:
   `see(quality=true)`.
2. **Круговой текст**: нужен детектор «мусорных OCR-кластеров» или региональная подсказка
   (см. критерий 1).
3. **Деплой бинарников десктоп-приложения**: при следующей сборке инсталлятора новые
   брокер/MCP попадут в AppData автоматически.
4. `eye-worker.mjs` в деплое отличается от репо (доведено до сведения; в этом изменении
   не трогался).
