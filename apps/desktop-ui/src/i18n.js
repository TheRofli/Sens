...
const messages = {
  ru: {
    ...
    "toggle.localDesc": "Детерминированный анализ на устройстве: OCR, layout, объекты, сцена, внимание. Без API.",
    ...
    "provider.hint": "Локальный стек — детерминированный анализ на устройстве. Облачные провайдеры нужны только для сравнения изображений и работы с артефактами Eye.",
    "sight.localStack": "Локальный стек без API: OCR (кириллица + латиница), layout-блоки, объекты (YOLO), классификация сцены (CLIP), карта внимания и перекрёстная проверка слоёв. Все модели работают на вашем CPU — интернет и ключи не нужны.",
    "sight.alwaysMax": "Режимы не требуются — анализ всегда выполняется на максимум: полный OCR, все объекты, сцена и зоны внимания для любого изображения.",
    "field.visionPack": "Семантика (локальные VLM-подписи)",
    "visionPack.lite": "Быстрая — SmolVLM2-500M (~0.7 ГБ RAM)",
    "visionPack.quality": "Качественная — SmolVLM2-2.2B (~2 ГБ RAM)",
    "visionPack.qualityLarge": "Максимум — Qwen2.5-VL-3B (~3.5 ГБ RAM)",
    "visionPack.hint": "Дефолт для всех запросов; модель может переопределить аргументами sens_see (pack/quality/fast). Нескачанный пак деградирует до lite; скачать: scripts/download-vision-models.py.",
  ...
  en: {
    ...
    "toggle.localDesc": "Deterministic on-device analysis: OCR, layout, objects, scene, attention. No API.",
    ...
    "provider.hint": "Local stack is deterministic on-device analysis. Cloud providers are only needed for image comparison and Eye artifact flows.",
    "sight.localStack": "Local stack, no API: OCR (cyrillic + latin), layout blocks, objects (YOLO), scene classification (CLIP), attention map, and cross-layer verification. All models run on your CPU — no internet or keys needed.",
    "sight.alwaysMax": "No modes needed — analysis always runs at maximum: full OCR, all objects, scene, and attention zones for any image.",
    "field.visionPack": "Semantics (local VLM captions)",
    "visionPack.lite": "Fast — SmolVLM2-500M (~0.7 GB RAM)",
    "visionPack.quality": "Quality — SmolVLM2-2.2B (~2 GB RAM)",
    "visionPack.qualityLarge": "Maximum — Qwen2.5-VL-3B (~3.5 GB RAM)",
    "visionPack.hint": "Default for all requests; a model can override it via sens_see arguments (pack/quality/fast). A pack that is not downloaded degrades to lite; download via scripts/download-vision-models.py.",
