...
$manifest = [ordered]@{
    version = $appVersion
    notes = "Sens ${appVersion}: Touch - delegate self-contained work to cheap worker models (OpenRouter/DeepSeek/custom OpenAI-compatible) with roles, budgets, broker-issued evidence receipts and deterministic fact checking. Workers have no secrets, no direct filesystem and no network; the broker executes every privileged action. Opt-in: disabled by default, with an in-app kill switch."
